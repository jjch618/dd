...
unsigned __height(_NodePtr __x) {
	...
	unsigned __leftHeight = __height(__x->__left_);
	unsigned __rightHeight = __height(__x->__right_);

	return 1 + (__leftHeight > __rightHeight ? __leftHeight : __rightHeight);
}
...
unsigned __size(_NodePtr __x) {
	...
	unsigned __leftSize = __size(__x->__left_);
	unsigned __rightSize = __size(__x->__right_);

	return 1 + __leftSize + __rightSize;
}
...
std::pair<_NodePtr, bool> __insertBST(_NodePtr& __root, const _Tp& key) {
	...
	else if (key < __q->__key_) __q->__left_ = newNode;
...
_NodePtr __eraseBST(_NodePtr& __root, const _Tp& key) {
	// __p에 삭제할 노드, __q에 삭제할 노드의 부모 노드를 저장
	_NodePtr __p = __root;
	_NodePtr __q = nullptr;
	...
	// 만약 삭제할 key값의 노드가 없으면 nullptr 반환
