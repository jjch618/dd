...
_NodePtr __tree_min(_NodePtr __x) {
	...
	_NodePtr __n = __x;
	while (__n->__left_ != nullptr) __n = __n->__left_;
	return __n;
}
...
_NodePtr __tree_max(_NodePtr __x) {
	...
	if (__x == nullptr) return nullptr;
	_NodePtr __n = __x;
	while (__n->__right_ != nullptr) __n = __n->__right_;
	return __n;
}
...
std::pair<_NodePtr, bool> __insertBST(_NodePtr& __root, const _Tp& key) {
	...
	_NodePtr __n = new Node<_Tp>(key);
	if (__q == nullptr) __root = __n;
	else if (key < __q->__key_) __q->__left_ = __n;
	else __q->__right_ = __n;
	return std::pair<_NodePtr, bool>(__n, true);
}
...
_NodePtr __eraseBST(_NodePtr& __root, const _Tp& key) {
	...
	// Case 3: Node has two children - use replacement strategy
	bool flag = true;
	_NodePtr __r;
	unsigned left_height = __height(__p->__left_);
	unsigned right_height = __height(__p->__right_);
	...
	_Tp replacement_key = __r->__key_;
	_NodePtr deleted_node = nullptr;
	if (flag) {
		deleted_node = __eraseBST(__p->__left_, replacement_key);
	} else {
		deleted_node = __eraseBST(__p->__right_, replacement_key);
	}
	__p->__key_ = replacement_key;
	
	// Return the node that was actually deleted (the replacement node)
	// If the replacement node had no children or one child, it was already returned
	// If it had two children, we recursively handled it, so we return __r
	...
	*/
	...
	public: // Modifier
		std::pair<const_pointer, bool> insert(const key_type& key) {
			...
			auto result = __insertBST(__root_, key);
			return std::pair<const_pointer, bool>(result.first, result.second);
		}
